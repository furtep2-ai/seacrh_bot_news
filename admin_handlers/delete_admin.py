...
async def delete_admin_cmd(message: types.Message, state: FSMContext, current_user: User = None):
    if not current_user:
    ...
    if not current_user.is_admin:
        ...
        return
    await message.answer("Введите ID пользователя, которого хотите лишить админских прав.")
    await state.set_state(AdminStates.waiting_for_user_id)

@router.message(AdminStates.waiting_for_user_id)
async def delete_process_id_user(message: types.Message, state: FSMContext, current_user: User = None):
    ...
    result = await make_admin(user_id , 1)
